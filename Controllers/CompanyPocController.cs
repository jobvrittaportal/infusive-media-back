...
namespace Infusive_back.Controllers
{
  ...
  public class CompanyPocController : ControllerBase
  {
    ...
    public IActionResult GetPoc([FromQuery] int? skip, [FromQuery] int? limit, [FromQuery] string? search, int companyId)
    {
      try
      {
        var query = db.CompanyPoc.Include(u => u.Company)
            .Select(user => new
            {
              user.Id,
    ...
    public IActionResult UpdatePoc([FromBody] UpdatePocDto updatePoc)
    {
      try
      {
        ...
        CompanyPoc? alreadyExist = db.CompanyPoc.SingleOrDefault(f =>( f.Email.Trim().Equals(updatePoc.Email.Trim(), StringComparison.CurrentCultureIgnoreCase) || f.PhoneNumber.Trim().Equals(updatePoc.PhoneNumber.Trim(), StringComparison.CurrentCultureIgnoreCase)) && f.Id != updatePoc.Id);
        ...
        companyPoc.Designation = updatePoc.Designation;
